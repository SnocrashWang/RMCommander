...
from RMUL.environment import ActionRMUL
from RMUL.config.robot_config import RMUL_ROBOT_TYPE_LIST
from utils.config.game_config import GameTeam
from utils.config.robot_config import ROBOT_ID, RobotType
from torch.distributions import Normal, Categorical

class PPONetworkRMUL(nn.Module):
    def __init__(self, state_size: int):
        super(PPONetworkRMUL, self).__init__()
        ...
        self.target_network = nn.Sequential(
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Linear(64, 1 + len(RMUL_ROBOT_TYPE_LIST))  # 输出可能目标的logits
        )
...
class PPOAgentRMUL:
    def __init__(
        ...
        self.target_robot_list = [RobotType.NONE] + RMUL_ROBOT_TYPE_LIST
        ...
        # 创建网络并移动到指定设备
        self.network = PPONetworkRMUL(state_size).to(self.device)
    ...
    def _process_network_output(self, nav_mean, nav_std, attack_logits, target_logits) -> Dict[str, ActionRMUL]:
        """将网络输出转换为实际动作"""
        # 处理导航坐标
        ...
        return {
            ROBOT_ID[self.team][robot_type]: ActionRMUL(
    ...
    def act(self, state: np.ndarray) -> Dict[str, ActionRMUL]:
        """选择动作"""
