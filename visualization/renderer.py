...
from utils.game_config import GameState, GameTeam, TEAM_COLORS, ROBOT_COLORS
from utils.exp_prop_config import LEVEL_NEED_EXP
...
class Renderer:
    ...
    def render(self, env_state, obstacles, show_grid=False):
        """渲染环境"""
        # 清空屏幕
        self.screen.fill(env_config.COLOR_BACKGROUND)
        ...
        # 绘制可移动栅格（如果启用）
        if show_grid:
            self._draw_grid(env_state["robots"][0].grid_map)
            self._draw_path(env_state["robots"][0])

    def _draw_center_zone(self):
        """绘制中心区域"""
        center_zone_rect = env_config.CENTER_ZONE_RECT
        pygame.draw.rect(self.screen, env_config.COLOR_CENTER_ZONE, center_zone_rect)  # 更浅的绿色

    def _draw_walls(self):
        """绘制围墙"""
        # 上墙
        pygame.draw.rect(self.screen, env_config.COLOR_WALL, (0, 0, env_config.SCALE * env_config.FIELD_WIDTH, 10))
        # 下墙
        bottom_y = env_config.SCALE * env_config.FIELD_HEIGHT - 10
        pygame.draw.rect(self.screen, env_config.COLOR_WALL, (0, bottom_y, env_config.SCALE * env_config.FIELD_WIDTH, 10))
        # 左墙
        pygame.draw.rect(self.screen, env_config.COLOR_WALL, (0, 0, 10, env_config.SCALE * env_config.FIELD_HEIGHT))
        # 右墙
        right_x = env_config.SCALE * env_config.FIELD_WIDTH - 10
        pygame.draw.rect(self.screen, env_config.COLOR_WALL, (right_x, 0, 10, env_config.SCALE * env_config.FIELD_HEIGHT))
    ...
    def _draw_obstacles(self, obstacles):
        ...
        for obs in obstacles:
            ...
            pygame.draw.polygon(self.screen, env_config.COLOR_OBSTACLE, [to_px(v1), to_px(v2), to_px(v3), to_px(v4)])
    ...
    def _draw_tank(self, robot):
        ...
        # 绘制机器人主体
        pygame.draw.circle(self.screen, ROBOT_COLORS[robot.team], (x, y), radius)
        ...
        # 绘制机器人标识（使用小字体）
        team_text = self.tiny_font.render(f"{robot.id.value}", True, env_config.COLOR_TEXT)
        self.screen.blit(team_text, (x - team_text.get_width() / 2, y + radius * 1.5))


        # 绘制数据条
        bar_width = env_config.SCALE * 0.5
        bar_height = env_config.SCALE * 0.08
        # 血量条
        hp_bar_x = x - bar_width / 2
        hp_bar_y = y - radius * 1.5 - bar_height * 2
        current_hp_width = int(bar_width * robot.hp / robot.max_hp)
        pygame.draw.rect(self.screen, TEAM_COLORS[robot.team],
                        (hp_bar_x, hp_bar_y, current_hp_width, bar_height))
        hp_text = self.tiny_font.render(f"{robot.hp:>3d}/{robot.max_hp:>3d}", True, env_config.COLOR_TEXT)
        ...
        pygame.draw.rect(self.screen, env_config.COLOR_HEAT_BAR,
                        (heat_bar_x, heat_bar_y, current_heat_width, bar_height))
        heat_text = self.tiny_font.render(f"{int(robot.heat)}/{int(robot.max_heat)}", True, env_config.COLOR_TEXT)
        ...
        current_exp_width = int(bar_width * (robot.exp - LEVEL_NEED_EXP[robot.level]) / exp_need_to_level_up)
        pygame.draw.rect(self.screen, env_config.COLOR_EXP_BAR,
                        (exp_bar_x, exp_bar_y, current_exp_width, bar_height))
        exp_text = self.tiny_font.render(f"{robot.exp - LEVEL_NEED_EXP[robot.level]:>3d}/{exp_need_to_level_up:>3d}", True, env_config.COLOR_TEXT)
        self.screen.blit(exp_text, (exp_bar_x + bar_width / 2 - exp_text.get_width() / 2, exp_bar_y + bar_height / 2 - exp_text.get_height() / 2))
        # 等级
        level_text = self.tiny_font.render(f"Lv.{robot.level:>2d}", True, env_config.COLOR_TEXT)
    ...
    def _draw_attack_line(self, attack_line):
        ...
        """
        if attack_line:
            draw_dashed_line(self.screen, env_config.SCALE, (0, 255, 0), attack_line[0], attack_line[1], int(env_config.SCALE * 0.02), int(env_config.SCALE * 0.05), int(env_config.SCALE * 0.1))

    def _draw_progress_bars(self, game_state):
        """绘制进度条"""
        bar_height = env_config.SCALE * env_config.FIELD_HEIGHT * 0.02
        screen_width = env_config.SCALE * env_config.FIELD_WIDTH
        bar_width = screen_width * 0.4

        # 红队进度条
        pygame.draw.rect(self.screen, env_config.COLOR_PROGRESS_BAR_BG, (10, 10, bar_width, bar_height))
        progress_width = int(bar_width * (game_state.center_zone_progress[GameTeam.RED] / env_config.OCCUPATION_TARGET))
        pygame.draw.rect(self.screen, TEAM_COLORS[GameTeam.RED],
                         (10, 10, progress_width, bar_height))

        # 蓝队进度条
        pygame.draw.rect(self.screen, env_config.COLOR_PROGRESS_BAR_BG, (screen_width - bar_width - 10, 10, bar_width, bar_height))
        progress_width = int(bar_width * (game_state.center_zone_progress[GameTeam.BLUE] / env_config.OCCUPATION_TARGET))
        pygame.draw.rect(self.screen, TEAM_COLORS[GameTeam.BLUE], 
                         (screen_width - bar_width - 10 + (bar_width - progress_width), 10, progress_width, bar_height))

        # 进度文本
        text1 = self.font.render(
            f"Team {GameTeam.RED.value}: {int(game_state.center_zone_progress[GameTeam.RED] / env_config.OCCUPATION_TARGET * 100):>3d}%",
            True, env_config.COLOR_TEXT
        )
        text2 = self.font.render(
            f"Team {GameTeam.BLUE.value}: {int(game_state.center_zone_progress[GameTeam.BLUE] / env_config.OCCUPATION_TARGET * 100):>3d}%",
            True, env_config.COLOR_TEXT
        )
        ...
        text3 = self.font.render(f"Time: {min:02d}:{sec:02d}", True, env_config.COLOR_TEXT)
    ...
    def _draw_info(self, game_state, show_grid=False):
        ...
        for i, text in enumerate(controls):
            text_control = self.font.render(text, True, env_config.COLOR_TEXT)
            self.screen.blit(text_control, (10, env_config.SCALE * env_config.FIELD_HEIGHT - (len(controls) - i + 1) * 30))

        # Show grid status
        grid_status = "ON" if show_grid else "OFF"
        grid_text = self.font.render(f"Movable Grid: {grid_status}", True, env_config.COLOR_TEXT)
        self.screen.blit(grid_text, (10, env_config.SCALE * env_config.FIELD_HEIGHT - 30))

        # Show game state
        if game_state.state == GameState.RED_TEAM_WIN:
            win_text = self.large_font.render("Team Red Wins!", True, TEAM_COLORS[GameTeam.RED])
            ...
            win_text = self.large_font.render("Team Blue Wins!", True, TEAM_COLORS[GameTeam.BLUE])
            text_rect = win_text.get_rect(center=(env_config.SCALE * env_config.FIELD_WIDTH // 2, 
                                                 env_config.SCALE * env_config.FIELD_HEIGHT // 2))
            self.screen.blit(win_text, text_rect)
        elif game_state.state == GameState.DRAW:
            win_text = self.large_font.render("Draw!", True, env_config.COLOR_TEXT)  # 灰色字体
            text_rect = win_text.get_rect(center=(env_config.SCALE * env_config.FIELD_WIDTH // 2, 
                                                 env_config.SCALE * env_config.FIELD_HEIGHT // 2))
            self.screen.blit(win_text, text_rect)
